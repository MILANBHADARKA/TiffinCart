...
function EditTiffinItem() {
  const { id: kitchenId, itemId } = useParams();
  ...
  }, [isAuthenticated, isLoading, user, router, kitchenId, itemId]);
  ...
  const fetchData = async () => {
    try {
      ...
      const itemResponse = await fetch(`/api/seller/kitchen/${kitchenId}/menu/${itemId}`, {
        credentials: 'include'
      });
  ...
  const onSubmit = async (data) => {
    ...
    if (data.ingredients) {
      data.ingredients = data.ingredients.split(',').map(item => item.trim()).filter(item => item);
    } else {
      data.ingredients = [];
    }
    ...
    // Add image data if uploaded new image
    if (uploadedImageUrl) {
    ...
    try {
      const response = await fetch(`/api/seller/kitchen/${kitchenId}/menu/${itemId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (result.success) {
        setMessage('Tiffin item updated successfully!');
        setMessageType('success');
        setTimeout(() => {
          router.push(`/seller/kitchen/${kitchenId}/menu`);
        }, 1500);
      } else {
        setMessage(result.error || 'Failed to update tiffin item');
        setMessageType('error');
      }
    } catch (error) {
      console.error('Error updating tiffin item:', error);
      setMessage('Network error. Please try again.');
      setMessageType('error');
    } finally {
  ...
  if (!isAuthenticated || user?.role !== 'seller' || !menuItem) {
    return (
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className={`mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
              Update your tiffin item details
            </p>
          </div>
          <Link
            href={`/seller/kitchen/${kitchenId}/menu`}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-6">
                    ...
                    {!imagePreview ? (
                      ...
                      <div className="relative">
                        ...
                        <button
                          type="button"
                          onClick={() => fileInputRef.current?.click()}
                          className="absolute bottom-2 right-2 bg-blue-500 text-white rounded-lg px-3 py-1 text-xs hover:bg-blue-600"
                        >
                          Change Photo
                        </button>
                        <input
            ...
            <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
              ...
              <button
                ...
                className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-2.5 rounded-lg font-medium hover:from-blue-600 hover:to-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center"
              >
                {isSubmitting ? (
                  <>
